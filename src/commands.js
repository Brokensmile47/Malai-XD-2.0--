...
*Made by Kimani Samuel*`;
  ...
  add({ name: 'getpp', aliases: ['dp','profile','whois','userinfo','about'], category: 'utility',
    desc: 'Get user profile picture and details',
    usage: '@user | +254... | (reply)',
    handler: async (ctx) => {
      try {
        // Get target JID
        let targetJid = '';
        ...
        function detectCountry(n = '') {
          ...
          const map = [
            ...
            ['27','South Africa 🇿🇦'],['233','Ghana 🇬🇭'],['234','Nigeria 🇳🇬'],
            ['44','United Kingdom 🇬🇧'],['33','France 🇫🇷'],['49','Germany 🇩🇪'],
            ['91','India 🇮🇳'],['92','Pakistan 🇵🇰'],['86','China 🇨🇳'],['81','Japan 🇯🇵'],
            ['1','United States 🇺🇸'],['55','Brazil 🇧🇷'],['52','Mexico 🇲🇽'],
          ].sort((a,b) => b[0].length - a[0].length);
          for (const [prefix, country] of map) if (n.startsWith(prefix)) return country;
          return 'Unknown 🌍';
        }
        ...
        let ppBuffer = null;
        try {
          const ppUrl = await ctx.sock.profilePictureUrl(targetJid, 'image').catch(() => null);
          if (ppUrl) {
        ...
        const num_display = num || 'unknown';
        const country = detectCountry(num_display);
        const pushName = ctx.message?.pushName || 'User';
...
╰━━━━━━━━━━━━━━━━━━╯`;
        ...
        if (ppBuffer) {
          ...
          await reply(ctx, card + '\n\n🖼️ _No profile picture or private_', { mentions: [targetJid] });
        }
      } catch (err) {
...
${madeByFooter(cfg)}`;
  ...
  // ─── AUTOREAD ─────────────────────────────────────────────────────────────
  add({ name: 'autoread', category: 'owner', ownerOnly: true, desc: 'Mark all messages as read instantly', usage: '<on|off|status>', handler: async (ctx) => {
    const st = getState();
    const action = (ctx.args[0] || 'status').toLowerCase();
    if (action === 'on') { setToggle(st, 'autoread', true); saveState(st); return reply(ctx, '✅ *Autoread ON* — all messages marked as read instantly.'); }
    if (action === 'off') { setToggle(st, 'autoread', false); saveState(st); return reply(ctx, '❌ *Autoread OFF*'); }
    return reply(ctx, `*Autoread:* ${isToggleEnabled(st, 'autoread') ? '✅ ON' : '❌ OFF'}\n\n${getConfig().prefix}autoread on/off`);
  }});
